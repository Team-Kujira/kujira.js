    ...
    toJSON(message) {
        ...
        if (message.aggregate_exchange_rate_prevotes) {
            obj.aggregate_exchange_rate_prevotes =
                message.aggregate_exchange_rate_prevotes.map((e) => e ? AggregateExchangeRatePrevote.toJSON(e) : undefined);
        }
        else {
            obj.aggregate_exchange_rate_prevotes = [];
        }
        if (message.aggregate_exchange_rate_votes) {
            obj.aggregate_exchange_rate_votes =
                message.aggregate_exchange_rate_votes.map((e) => e ? AggregateExchangeRateVote.toJSON(e) : undefined);
        }
        else {
            obj.aggregate_exchange_rate_votes = [];
        }
        ...
        while (reader.pos < end) {
            ...
            switch (tag >>> 3) {
                case 1:
                    ...
                    break;
                case 2:
                    message.miss_counter = longToNumber(reader.uint64());
                    break;
                default:
                    reader.skipType(tag & 7);
                    break;
